...
export class RegistroPage implements OnInit {
  ...
  constructor(
    ...
    this.platform.backButton.subscribeWithPriority(0, () => {
      //cuando apreto el boton volver de la pantalla de android, dejo de intentar scanear el codigo
  ...
  registrar() {
    if (
      ...
      this.servicio
        .registerEmail(this.correo, this.clave)
        .then(() => {
          if (this.file != null) {
            this.servicio
              .uploadPhoto(this.file, `cliente/${this.correo}`)
              .then((datos) => {
                this.url = <string>datos;
                this.servicio.createDocInDB(
                  "cliente",
                  this.correo,
                  this.toJSON()
                );
            ...
            this.url = "default";
            this.servicio.createDocInDB("cliente", this.correo, this.toJSON());
          }
  ...
  validarCorreo(): boolean {
    ...
    if (!regex.test(this.correo)) {
      this.s_utilidad.textoMostrar(
        "#modal-error-text-p",
        "El campo debe ser de tipo correo",
        "#modal-error",
        ".container-registro"
      );
    } else if (this.correo == "") {
  ...
  validarNombreApellido(): boolean {
    ...
    if (this.nombre == "") {
      ...
      !regexLetras.test(this.nombre) ||
      !regexLetras.test(this.apellido)
    ) {
      this.s_utilidad.textoMostrar(
        "#modal-error-text-p",
        "El campo debe incluir solo letras",
        "#modal-error",
        ".container-registro"
      );
    } else {
  ...
  validarDni(): boolean {
    ...
    if (this.dni == undefined) {
      ...
      this.s_utilidad.textoMostrar(
        "#modal-error-text-p",
        "El campo dni debe ser de tipo numerico",
        "#modal-error",
        ".container-registro"
      );
      ...
      this.s_utilidad.textoMostrar(
        "#modal-error-text-p",
        "El campo dni debe poseer 8 numeros",
        "#modal-error",
        ".container-registro"
      );
    } else {
  ...
  selectPhotoInPhotolibrary() {
    this.servicio
      .choosePhotoLibrary()
      .then((foto) => (this.file = <string>foto));
  }
